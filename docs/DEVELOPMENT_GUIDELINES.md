...
**Bluetooth transport logs are opt-in for release builds.** They are gated where
they are opened, so a shipped build writes nothing to a user's disk unasked. Each
