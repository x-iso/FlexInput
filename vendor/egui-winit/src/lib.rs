...
pub fn create_winit_window_attributes(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // FLEXINPUT PATCH: transparent taskbar-less viewports (FlexInput's
        // screen overlay) get WS_EX_NOREDIRECTIONBITMAP — the window then has
        // NO legacy GDI redirection surface and is composited purely from its
        // DirectComposition swapchain visual. Without this, the never-painted
        // redirection surface itself gets composited on Win11 26H1 (+ AMD) as
        // an opaque white sheet frozen at an early creation-time size, UNDER
        // the swapchain content — unfixable post-hoc since this extended
        // style is creation-time-only (which is why the vendored copy exists;
        // egui's ViewportBuilder can't express it). Keyed on
        // transparent+skip-taskbar so normal windows are untouched. This is
        // how every healthy transparent window does it (Chromium, XAML
        // islands); winit also skips its blur-behind hack when set.
        if transparent.unwrap_or(false) && !_taskbar.unwrap_or(true) {
