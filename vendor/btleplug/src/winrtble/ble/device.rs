...
impl BLEDevice {
    pub async fn new(
        ...
        // FLEXINPUT PATCH: keep unpaired BLE links from being reclaimed.
        //
        // Upstream never sets this. Per Microsoft's docs a connection is
        // initiated — and held — by setting `MaintainConnection`, by uncached
        // service discovery, or by a read/write against the device. `connect()`
        // below does only the discovery, which *opens* a link but does not ask
        // Windows to keep it. Incoming GATT notifications do not count as the
        // activity Windows looks for, so a device that only streams (a
        // gamepad, a sensor) is torn down after roughly 30 seconds even though
        // data is flowing the whole time.
        //
        // Found while bringing up Joy-Con 2 support: controllers dropped at a
        // near-constant ~29 s regardless of scanning behaviour, connection
        // count, or pairing state.
        //
        // Deliberately non-fatal — if the property cannot be set we still want
        // the connection, just with upstream's lifetime. Reported via
        // `eprintln!` rather than `trace!` because the whole point of the patch
        // is being able to tell whether it took effect; a trace-level line is
        // invisible under a default log filter and leaves you guessing.
        match gatt_session.SetMaintainConnection(true) {
            Ok(()) => eprintln!("[btleplug-patch] MaintainConnection=true set"),
            Err(e) => eprintln!("[btleplug-patch] MaintainConnection FAILED: {:?}", e),
        }
        ...
        let connection_status_handler =
            TypedEventHandler::<BluetoothLEDevice, _>::new(move |sender, _| {
                if let Some(sender) = sender.as_ref() {
                    ...
                    // FLEXINPUT PATCH: ground truth for *when Windows itself*
                    // considers the link up or down. Distinguishing "the OS
                    // dropped the connection" from "our GATT objects were
                    // disposed underneath us" is otherwise guesswork — both
                    // surface identically as failed writes.
                    eprintln!(
                        "[btleplug-patch] ConnectionStatusChanged -> connected={is_connected}"
                    );
...
impl Drop for BLEDevice {
    fn drop(&mut self) {
        // FLEXINPUT PATCH: this Drop closes every GattDeviceService and the
        // BluetoothLEDevice itself, which invalidates any GattCharacteristic a
        // caller still holds — every later use then fails with RO_E_CLOSED
        // (0x80000013, "The object has been closed"). Since `connect()`
        // overwrites `shared.device` with a fresh BLEDevice, a second connect
        // silently destroys the first connection's characteristics.
        //
        // If this line appears while a caller believes it is still connected,
        // the disconnection came from inside this process, not from the radio.
        eprintln!("[btleplug-patch] BLEDevice::drop — closing services + device");
        let result = self
            .gatt_session
