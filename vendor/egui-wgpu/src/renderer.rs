    ...
    pub fn update_buffers(
        ...
        if index_count > 0 {
            ...
            let Some(mut index_buffer_staging) = index_buffer_staging else {
                // FlexInput vendor patch: stock egui-wgpu `panic!`s here. A
                // `None` staging buffer means the GPU device is lost (a
                // fullscreen game reset the device, a driver TDR, or VRAM
                // exhaustion). Panicking aborts the whole process. Instead we
                // raise the process-global `GPU_LOST` flag and skip this frame;
                // the host app sees the flag and relaunches cleanly. Logged at
                // error once is enough — the flag latches and the app exits.
                crate::GPU_LOST.store(true, std::sync::atomic::Ordering::SeqCst);
                log::error!(
                    "GPU device lost while staging index data (index count: {index_count}, \
                     required size: {required_index_buffer_size} bytes). Skipping frame and \
                     signalling GPU_LOST."
                );
                return Vec::new();
            };
        ...
        if vertex_count > 0 {
            ...
            let Some(mut vertex_buffer_staging) = vertex_buffer_staging else {
                // FlexInput vendor patch: see the matching index-buffer site
                // above. Device lost mid-frame — raise GPU_LOST and skip the
                // frame instead of panicking the whole process.
                crate::GPU_LOST.store(true, std::sync::atomic::Ordering::SeqCst);
                log::error!(
                    "GPU device lost while staging vertex data (vertex count: {vertex_count}, \
                     required size: {required_vertex_buffer_size} bytes). Skipping frame and \
                     signalling GPU_LOST."
                );
                return Vec::new();
            };
