...
/// Process-global flag set by the renderer when the GPU device is lost
/// mid-frame (a fullscreen game resetting the graphics device, a driver TDR,
/// or VRAM exhaustion). Stock egui-wgpu `panic!`s in this situation because
/// `wgpu::Queue::write_buffer_with` returns `None` and the renderer has no way
/// to recover the `RenderState`; that aborts the whole process with a
/// backtrace.
///
/// This vendored copy instead skips the affected frame and raises this flag so
/// the host application can react gracefully — FlexInput saves its workspace and
/// relaunches a fresh process (eframe 0.33 exposes no API to rebuild the device
/// in place). The flag latches `true`; it is never cleared, because once the
/// device is gone every subsequent frame is also dead. Read it once per frame
/// from your `eframe::App::update`.
pub static GPU_LOST: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
