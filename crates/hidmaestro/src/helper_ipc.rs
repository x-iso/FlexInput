...
pub enum Request {
    ...
    ///
    /// Older builds also sent `poll_interval_ms` for FlexInput's rebuilt XUSB
    /// companion. The stock companion replaced that with an input doorbell, and
    /// serde ignores the stray field, so the two sides stay compatible either way.
