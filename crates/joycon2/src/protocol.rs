...
pub mod feature {
    ...
    /// gyro is confirmed.
    /// ⛔ **0x2f, and NOT changed to 0x07 without hardware saying so.**
    ///
    /// Two sources disagree and only one of them is a capture:
    ///
    /// * this repository recorded `0c 91 01 02 00 04 00 00 2f 00 00 00` as the
    ///   payload a working reference writes, and that without [`IMU_RAW`] the
    ///   standard accel/gyro block at `0x30..0x3c` stays entirely zero;
    /// * `trevlars/switch2-controllers-linux` defaults to
