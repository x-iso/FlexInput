...
fn connect_and_init(
    ...
    for (what, sub_id) in [
        ...
        //
        // Every layer above this has been reporting success and nothing has
        // happened: the ATT write is acknowledged, the CCCD is acknowledged,
        // the rate descriptor is acknowledged, and `0x000a` has never sent a
        // byte. All that proves is that the writes reached their attributes.
        //
        // The command channel has a REPLY channel — subscribed since init, and
        // never once read for this command. Three outcomes and they need three
        // different fixes: a reply carrying an error means `0x03` is understood
        // and refused, so the flags or the sequence are wrong; a reply carrying
        // success means it was accepted and the silence is somewhere else
        // entirely; no reply at all means this command family is not the one
        // this firmware speaks, and the numbering taken from the reference does
        // not apply here.
        //
        // Guessing between those three is what the last several rounds have
        // been. It costs one short drain to stop.
        ...
        while Instant::now() < deadline && !answered {
            for pkt in dongle.drain_acl(32) {
                ...
                let offset = if note.handle == jc::HANDLE_CMD_RESPONSE {
                    0
                } else {
                    protocol::CMD_RESP_HEADER_OFFSET
                };
                let parsed = protocol::parse_response(&note.value, offset, protocol::CMD_ENABLE);
                crate::dlog::imu(format_args!(
                    "{} motion enable {what} REPLY on {:#06x}: {} | {:02x?}",
                    side.display_name(),
                    note.handle,
                    match parsed {
                        Some((h, data)) => format!(
                            "cmd {:#04x} sub {:#04x} ack {:#04x} data {data:02x?}",
                            h.cmd, h.subcmd, h.ack
                        ),
                        None => "not a reply to 0x03 (or a header this parser \
                                 does not recognise)"
                            .to_string(),
                    },
                    &note.value[..note.value.len().min(32)],
                ));
                answered = true;
                break;
            }
        ...
        if !answered {
            crate::dlog::imu(format_args!(
                "{} motion enable {what} — NO REPLY from the controller in 250 ms. \
                 The write landed; nothing answered it.",
                side.display_name()
            ));
        }
    }
